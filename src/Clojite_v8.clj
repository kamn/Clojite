...
(defn site-value 
  ...
  (let [enemy (if (and (not= (:owner site) my-id) (not= (:owner site) 0)) 255 0)
        str-value (if (zero? (:strength site)) -10000 1)
...
(defn get-lowest-direction2
  ...
  (let [north-site (get-north-site game-state site)
    ...
    (if (and (< (:strength site) (* 6 (:production site))) 
             ...
             (> (:strength 
               (if (has-foreign-sites game-state strongest-friend-site) 
                 (other-weak-site game-state strongest-friend-site) 
                   {:strength 10000})) 
                (+ (:strength site) (:strength strongest-friend-site))))
      [site :still]
      (if (nil? weak-site)
